...
function ensureAdminStyles(){if(document.getElementById("bingoAdminRuntimeStyles"))return;const style=document.createElement("style");style.id="bingoAdminRuntimeStyles";style.textContent=`.admin-category-nav{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin:12px 0 16px}.admin-cat{border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.045);color:inherit;border-radius:12px;padding:11px 8px;font-weight:800}.admin-cat.active{background:rgba(72,139,255,.2);border-color:rgba(72,139,255,.55)}.admin-panel-head{display:flex;justify-content:space-between;gap:12px;align-items:flex-start;margin-bottom:10px}.admin-search-row,.invite-link-row{display:flex;gap:8px;align-items:center;margin:10px 0}.admin-search-row input,.invite-link-row input{flex:1;min-width:0}.admin-row .request-details{margin-top:10px;padding:10px;border-radius:10px;background:rgba(255,255,255,.035);display:grid;gap:5px}.admin-row .inline-edit{display:grid;gap:8px;margin-top:10px}.admin-actions{display:flex;gap:6px;flex-wrap:wrap}.primary-mini{background:rgba(60,180,110,.18);border-color:rgba(60,180,110,.5)}.danger-mini{background:rgba(220,70,70,.16);border-color:rgba(220,70,70,.5)}.profile-invite-card{margin-top:14px;padding:16px;border-radius:16px;background:rgba(255,255,255,.045);border:1px solid rgba(255,255,255,.08)}.invite-stat{display:flex;justify-content:space-between;padding:10px 0;font-size:16px}.game-cards{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.game-cards .game-card:only-child{grid-column:1/-1;max-width:350px;width:100%;justify-self:center}.winner-panel .winner-card-wrap{margin-top:10px;display:flex;justify-content:center}.winner-panel .winner-card-wrap .game-card{max-width:340px;width:100%}`;document.head.appendChild(style)}
function selectAdminCategory(category){document.querySelectorAll(".admin-cat").forEach(b=>b.classList.toggle("active",b.dataset.adminCategory===category));document.querySelectorAll("[data-admin-panel]").forEach(p=>p.classList.toggle("hidden",p.dataset.adminPanel!==category))}
...
function renderWinnerPanel(data,myCards){const panel=document.getElementById("winnerNotice"),winners=data.winners||[];if(!winners.length||!["WINNER","RESULT","FINISHED"].includes(data.status)){panel.classList.add("hidden");return}panel.classList.remove("hidden");const ownId=Number(currentUser?.telegram_user_id||0),own=winners.filter(w=>Number(w.telegram_user_id)===ownId);let html='<div class="winner-title">🏆 BINGO WINNER</div>';if(own.length)html+='<div class="winner-own"><strong>🎉 YOU WON!</strong></div>';html+=winners.map(w=>`<div class="winner-own"><p><strong>Cartel #${w.card_number}</strong> · ${w.pattern_name||w.pattern}</p><p>Player: ${w.first_name||w.username||"Player"}</p><p>Prize: <strong>${money(w.prize_amount)}</strong></p><p class="winner-explain">Winning pattern: <strong>${w.pattern_name||w.pattern}</strong>. Winning cells are highlighted below.</p></div><div class="winner-card-wrap" id="winner-card-${w.card_number}"></div>`).join("");panel.innerHTML=html;winners.forEach(w=>{const host=document.getElementById(`winner-card-${w.card_number}`);if(!host)return;const card=myCards.find(c=>Number(c.card_number)===Number(w.card_number))||{card_number:w.card_number,card_data:w.card_data};if(card?.card_data)host.appendChild(renderMiniCard(card,data.called_numbers||[],w.pattern))})}
...
antLabels={dashboard:"Dashboard",deposits:"Deposits",withdrawals:"Withdrawals",player_balance:"Player Balance",player_bonus:"Player Bonus",bingo_rooms:"Bingo Games",bingo_settings:"Bingo Settings",commission:"Commission",settings:"General Settings",manage_admins:"Manage Admins",players:"Players",broadcast:"Broadcast"};
...
async function loadAdmin(){
    ...
    try{
        ...
        const allowed=k=>info.is_super_admin||(info.grants||[]).includes(k);
        ...
        if(allowed("players"))loadAdminPlayers();
        if(allowed("deposits"))safePanel("/api/admin/deposits",renderAdminDeposits,"adminDeposits");
        if(allowed("withdrawals"))safePanel("/api/admin/withdrawals",renderAdminWithdrawals,"adminWithdrawals");
        if(allowed("bingo_rooms"))safePanel("/api/admin/bingo-games",renderAdminRooms,"adminRooms");
        if(allowed("settings")||allowed("commission")||allowed("bingo_settings"))safePanel("/api/admin/settings",renderAdminSettings,"adminPatterns");
        if(info.is_super_admin)safePanel("/api/admin/admins",renderAdminList,"adminList");
        if(info.is_super_admin)safePanel("/api/admin/activity",d=>{document.getElementById("adminActivity").innerHTML=(d.activity||[]).map(x=>`<div class="admin-row"><strong>#${x.id} · ${x.action}</strong><span>${x.created_at||""}</span><small>Admin ${x.admin_telegram_id}${x.target_telegram_id?" · Target "+x.target_telegram_id:""}</small><small>${x.details||""}</small></div>`).join("")||'<div class="notice">No activity.</div>'},"adminActivity");
        const choices=document.getElementById("grantChoices");if(info.is_super_admin)choices.innerHTML=(info.admin_grants||[]).map(g=>`<label class="grant-choice"><input type="checkbox" value="${g}"> ${grantLabels[g]||g}</label>`).join("");
    }catch(e){document.getElementById("adminSection").classList.add("hidden");showToast(e.message)}
}

function renderAdminDeposits(d){const rows=d.deposits||[];document.getElementById("adminDeposits").innerHTML=rows.map(x=>`<div class="admin-row"><strong>#${x.id} · ${money(x.amount)}</strong><span>${x.first_name||""} ${x.last_name||""} ${x.username?"(@"+x.username+")":""}</span><div class="admin-actions"><button class="small-btn" data-action="view-request" data-kind="deposit" data-id="${x.id}">V</button><button class="small-btn" data-action="approve-deposit" data-id="${x.id}">Approve</button><button class="small-btn danger-mini" data-action="reject-deposit" data-id="${x.id}">Reject</button></div><div id="deposit-detail-${x.id}" class="request-details hidden"><b>Player:</b> ${x.first_name||""} ${x.last_name||""} · ${x.telegram_user_id}<b>Phone:</b> ${x.phone_number||"—"}<b>Bank:</b> ${x.bank_name||"—"}<b>Bank account:</b> ${x.account_name||"—"} / ${x.account_number||"—"}<b>Amount:</b> ${money(x.amount)}<b>Transaction ID:</b> ${x.transaction_id||"—"}<b>Submitted:</b> ${x.created_at||"—"}</div></div>`).join("")||'<div class="notice">No pending deposits.</div>'}
function renderAdminWithdrawals(d){const rows=d.withdrawals||[];document.getElementById("adminWithdrawals").innerHTML=rows.map(x=>`<div class="admin-row"><strong>#${x.id} · ${money(x.amount)}</strong><span>${x.first_name||""} ${x.last_name||""} ${x.username?"(@"+x.username+")":""}</span><div class="admin-actions"><button class="small-btn" data-action="view-request" data-kind="withdrawal" data-id="${x.id}">V</button><button class="small-btn" data-action="approve-withdrawal" data-id="${x.id}">Approve</button><button class="small-btn danger-mini" data-action="reject-withdrawal" data-id="${x.id}">Reject</button></div><div id="withdrawal-detail-${x.id}" class="request-details hidden"><b>Player:</b> ${x.first_name||""} ${x.last_name||""} · ${x.telegram_user_id}<b>Phone:</b> ${x.phone_number||"—"}<b>Bank:</b> ${x.bank_name||"—"}<b>Player account:</b> ${x.account_name||"—"} / ${x.account_number||"—"}<b>Amount:</b> ${money(x.amount)}<b>Submitted:</b> ${x.created_at||"—"}</div></div>`).join("")||'<div class="notice">No pending withdrawals.</div>'}
function renderAdminRooms(d){document.getElementById("adminRooms").innerHTML=(d.rooms||[]).map(r=>`<div class="admin-row" id="room-admin-${r.id}"><strong>Game #${r.id}</strong><span>Bet ${money(r.bet_amount)} · Max cartels ${r.max_cards} · ${Number(r.is_active)?"ACTIVE":"DISABLED"}</span><div class="admin-actions"><button class="small-btn" data-action="edit-room-inline" data-id="${r.id}">Edit</button></div></div>`).join("")||'<div class="notice">No games.</div>'}
function renderAdminSettings(d){adminCommission.value=d.commission_percent;settingRegistrationBonus.value=d.registration_bonus;settingReferralBonus.value=d.referral_bonus;settingFirstDeposit.value=d.first_deposit_required;settingMinWithdrawal.value=d.min_withdrawal;settingMaxWithdrawal.value=d.max_withdrawal;adminPatterns.innerHTML=(d.patterns||[]).map(p=>`<div class="admin-row pattern-row"><label><input type="checkbox" class="pattern-edit" data-key="${p.pattern_key}" ${Number(p.is_enabled)?"checked":""} disabled> <strong>${p.pattern_name}</strong></label><span>${Number(p.is_enabled)?"Enabled":"Disabled"}</span></div>`).join("")||'<div class="notice">No patterns.</div>'}
function renderAdminList(d){document.getElementById("adminList").innerHTML=(d.admins||[]).map(a=>`<div class="admin-row" id="admin-row-${a.telegram_user_id}" data-grants='${JSON.stringify(a.grants||[])}'><strong>${a.telegram_user_id}</strong><span>${Number(a.is_active)?"🟢 ACTIVE":"🔴 INACTIVE"}</span><span>${(a.grants||[]).map(g=>grantLabels[g]||g).join(", ")||"No grants"}</span><div class="admin-actions"><button class="small-btn" data-action="edit-admin" data-id="${a.telegram_user_id}">Edit</button></div><div class="admin-inline-editor hidden"></div></div>`).join("")||'<div class="notice">No additional administrators.</div>'}
async function loadAdminPlayers(){try{const q=adminPlayerSearch.value.trim();const d=await api("/api/admin/players?search="+encodeURIComponent(q));adminPlayers.innerHTML=(d.players||[]).map(p=>{const name=[p.first_name,p.last_name].filter(Boolean).join(" ")||"Player";return `<div class="admin-row" id="player-row-${p.telegram_user_id}"><strong>${name}</strong><span>ID ${p.telegram_user_id} · ${p.username?"@"+p.username:"no username"}</span><span>${p.is_blocked?"🔴 SUSPENDED":"🟢 ACTIVE"} · Referrals ${p.referral_count||0}</span><span>Total ${money(Number(p.main_balance)+Number(p.bonus_balance)+Number(p.withdrawable_balance))}</span><div class="admin-actions"><button class="small-btn" data-action="edit-player" data-id="${p.telegram_user_id}">Edit</button><button class="small-btn danger-mini" data-action="delete-player" data-id="${p.telegram_user_id}">Delete</button></div></div>`}).join("")||'<div class="notice">No players found.</div>'}catch(e){adminPlayers.innerHTML=`<div class="notice">${e.message}</div>`}}
async function adminAction(path,body=null,method="POST"){const options={method};if(body)options.body=JSON.stringify(body);const result=await api(path,options);showToast(result.message||"Saved successfully");await loadAdmin();await loadDashboard();return result}
function setSettingsEdit(enabled){[settingRegistrationBonus,settingReferralBonus,settingFirstDeposit,settingMinWithdrawal,settingMaxWithdrawal,adminCommission].forEach(x=>x.disabled=!enabled);editGeneralSettings.disabled=enabled;saveGeneralSettings.disabled=!enabled}
async function saveGeneralSettings(){try{await adminAction("/api/admin/settings/general",{registration_bonus:Number(settingRegistrationBonus.value),referral_bonus:Number(settingReferralBonus.value),first_deposit_required:Number(settingFirstDeposit.value),min_withdrawal:Number(settingMinWithdrawal.value),max_withdrawal:Number(settingMaxWithdrawal.value)});await adminAction("/api/admin/settings/commission",{percent:Number(adminCommission.value)});setSettingsEdit(false);showToast("Settings saved successfully")}catch(e){showToast(e.message)}}
async function adjustPlayerBalance(){try{await adminAction("/api/admin/player/"+adminBalanceType.value,{telegram_user_id:Number(adminPlayerId.value),amount:Number(adminBalanceAmount.value)});adminPlayerId.value="";adminBalanceAmount.value=""}catch(e){showToast(e.message)}}
async function addBingoGame(){try{await adminAction("/api/admin/bingo-games",{bet_amount:Number(newBetAmount.value),max_cards:Number(newMaxCards.value)});newBetAmount.value="";newMaxCards.value=""}catch(e){showToast(e.message)}}
async function sendBroadcast(){try{const fd=new FormData();fd.append("text",broadcastText.value.trim());fd.append("media_type",broadcastType.value);if(broadcastMedia.files[0])fd.append("media",broadcastMedia.files[0]);const r=await fetch(API_BASE_URL+"/api/admin/broadcast",{method:"POST",headers:{"X-Telegram-Init-Data":tg.initData},body:fd});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.detail||"Broadcast failed");broadcastText.value="";broadcastMedia.value="";broadcastResult.textContent=`Sent ${d.sent} · Failed ${d.failed} · Total ${d.total}`;broadcastResult.classList.remove("hidden");showToast("Broadcast sent successfully")}catch(e){showToast(e.message)}}
async function downloadBackup(){try{const r=await fetch(API_BASE_URL+"/api/admin/database-backup",{headers:{"X-Telegram-Init-Data":tg.initData}});if(!r.ok)throw new Error((await r.json().catch(()=>({}))).detail||"Backup failed");const blob=await r.blob();const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="bingo-backup.db";document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);showToast("Backup ready")}catch(e){showToast(e.message)}}
async function createAdmin(){try{const id=Number(newAdminId.value);const grants=[...document.querySelectorAll("#grantChoices input:checked")].map(x=>x.value);await adminAction("/api/admin/admins",{telegram_user_id:id,grants});newAdminId.value="";document.querySelectorAll("#grantChoices input").forEach(x=>x.checked=false)}catch(e){showToast(e.message)}}

document.addEventListener("DOMContentLoaded",async()=>{
    // Bind every DOM event only after the HTML has been parsed.
    // The previous release bound these at script-load time, which caused
    // a null-element TypeError and stopped the entire Mini App JavaScript.
    initTelegram();
    ...
    on("refreshBtn", "click", () => setPage(document.querySelector(".nav-btn.active")?.dataset.page || "dashboardPage"));
    on("backToDashboard", "click", () => setPage("dashboardPage"));
    on("backFromCalling", "click", closeCalling);
    on("submitDeposit", "click", submitDeposit);
    on("submitWithdrawal", "click", submitWithdrawal);
    on("adminRefresh", "click", loadAdmin);
    on("saveCommission", "click", saveCommission);
    on("saveGeneralSettings", "click", saveGeneralSettings);
    on("adminAdjustBalance", "click", adjustPlayerBalance);
    on("addBingoGame", "click", addBingoGame);
    on("createAdmin", "click", createAdmin);
    on("downloadBackup", "click", downloadBackup);

    // Main Mini App navigation.
...
async function loadProfile(){try{const d=await api("/api/profile"),p=d.profile||{};renderUser(d.user);document.getElementById("profileCard").innerHTML=`<div><span>Telegram ID</span><strong>${p.telegram_user_id||"—"}</strong></div><div><span>Username</span><strong>${p.username?"@"+p.username:"—"}</strong></div><div><span>Name</span><strong>${[p.first_name,p.last_name].filter(Boolean).join(" ")||"—"}</strong></div><div><span>Phone</span><strong>${p.phone_number||"—"}</strong></div><div><span>Referral code</span><strong>${p.referral_code||"—"}</strong></div><div><span>Total deposited</span><strong>${money(p.total_deposited)}</strong></div><div><span>Total withdrawn</span><strong>${money(p.total_withdrawn)}</strong></div><div><span>Total won</span><strong>${money(p.total_won)}</strong></div><div><span>Registered</span><strong>${p.registered_at||"—"}</strong></div>`;document.getElementById("referralCount").textContent=Number(d.referral?.count||0);const bot=d.bot_username||"";document.getElementById("inviteLink").value=bot&&p.referral_code?`https://t.me/${bot}?start=${encodeURIComponent(p.referral_code)}`:""}catch(e){showToast(e.message)}}
// Enhanced admin/profile controls are bound by the main DOMContentLoaded handler below.
function bindEnhancedControls(){
    ensureAdminStyles();
    document.querySelectorAll('.admin-cat').forEach(b=>b.addEventListener('click',()=>selectAdminCategory(b.dataset.adminCategory)));
    document.getElementById('editGeneralSettings')?.addEventListener('click',()=>setSettingsEdit(true));
    document.getElementById('saveGeneralSettings')?.addEventListener('click',saveGeneralSettings);
    document.getElementById('editPatterns')?.addEventListener('click',()=>{document.querySelectorAll('.pattern-edit').forEach(x=>x.disabled=false);const e=document.getElementById('editPatterns'),s=document.getElementById('savePatterns');if(e)e.disabled=true;if(s)s.disabled=false});
    document.getElementById('savePatterns')?.addEventListener('click',async()=>{try{for(const el of document.querySelectorAll('.pattern-edit'))await adminAction(`/api/admin/settings/patterns/${el.dataset.key}`,{enabled:el.checked});document.querySelectorAll('.pattern-edit').forEach(x=>x.disabled=true);const e=document.getElementById('editPatterns'),s=document.getElementById('savePatterns');if(e)e.disabled=false;if(s)s.disabled=true;showToast('Winning patterns saved successfully')}catch(e){showToast(e.message)}});
    document.getElementById('searchPlayers')?.addEventListener('click',loadAdminPlayers);
    document.getElementById('refreshPlayers')?.addEventListener('click',loadAdminPlayers);
    document.getElementById('sendBroadcast')?.addEventListener('click',sendBroadcast);
    document.getElementById('copyInviteLink')?.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(document.getElementById('inviteLink').value);showToast('Invite link copied')}catch(e){showToast('Copy failed')}});
    document.getElementById('shareInviteLink')?.addEventListener('click',()=>{const input=document.getElementById('inviteLink');if(input?.value)window.open(`https://t.me/share/url?url=${encodeURIComponent(input.value)}&text=${encodeURIComponent('Join me in Bingo Arena!')}`,'_blank')});
}
