...
async function api(path, options = {}) {
    ...
    for (let attempt = 1; attempt <= attempts; attempt++) {
        ...
        try {
            ...
            throw new Error(
                "Could not reach the Bingo server. Check the Render service and internet connection."
            );
        }
        ...
        if (!response.ok) {
            ...
            // A second GET attempt also helps when the Render service is
            // waking up or one request briefly loses the network connection.
            if (method === "GET" && attempt < attempts && response.status >= 500) {
                await new Promise(resolve => setTimeout(resolve, 350));
                continue;
            }
...
function ensureCallingStyles() {
    ...
    style.textContent = `
        ...
        .calling-letter.b { background: rgba(42,127,255,.35); color:#9dceff; }
        .calling-letter.i { background: rgba(43,180,99,.35); color:#9bf3c0; }
        .calling-letter.n { background: rgba(238,177,55,.35); color:#ffe08e; }
        .calling-letter.g { background: rgba(232,87,87,.35); color:#ffb4b4; }
        .calling-letter.o { background: rgba(164,91,232,.35); color:#dfbcff; }
        ...
        .winner-own {
            ...
            padding: 14px;
            border-radius: 14px;
            background: rgba(43,180,99,.14);
            border: 1px solid rgba(43,180,99,.35);
        }
...
function renderWinnerPanel(data, myCards) {
    ...
    const status = String(data.status || data.round?.status || "").toUpperCase();

    if (!winners.length || !["WINNER", "RESULT", "FINISHED"].includes(status)) {
        closeWinnerPopup();
        return;
    }
    ...
    let html = `
        <div class="winner-modal-card">
            <button type="button" class="winner-close" id="winnerCloseBtn" aria-label="Close">×</button>
            <div class="winner-title">🏆 BINGO WINNER</div>
            <div style="text-align:center;opacity:.78;font-size:13px;">
                Round #${data.round?.round_number || "—"} · ${winners.length} winner${winners.length === 1 ? "" : "s"}
            </div>
    `;

    if (own.length) {
        html += `<div class="winner-own">
            <strong>🎉 YOU WON!</strong>`;

        own.forEach(w => {
            html += `
                <p style="margin:8px 0 3px;">
                    Cartel #${w.card_number} · ${w.pattern_name || w.pattern}
                </p>
                <p style="margin:3px 0;">
                    Prize: <strong>${money(w.prize_amount)}</strong>
                </p>
                <p style="margin:8px 0 0;font-size:12px;opacity:.82;">
                    Your winning cells are highlighted on your cartel.
                </p>
            `;
        });
    ...
    html += `<div class="winner-list"><strong>Winner cartels</strong>`;

    winners.forEach(w => {
        const name = w.first_name || (w.username ? "@" + w.username : "Player");
        html += `
            <div class="winner-row">
                <span>Cartel #${w.card_number}</span>
                <span>${name}</span>
                <span>${w.pattern_name || w.pattern}</span>
                <span>${money(w.prize_amount)}</span>
            </div>
        `;
    });
    ...
    document.getElementById("winnerCloseBtn")?.addEventListener("click", closeWinnerPopup);

    // Highlight the winning cells on the player's own cartel.
    myCards.forEach(card => {
        const winner = winners.find(
            w => Number(w.telegram_user_id) === ownId &&
                 Number(w.card_number) === Number(card.card_number)
        );
        if (winner) card._winningPattern = winner.pattern;
    });
}
...
function renderCallingBoard(calledNumbers) {
    ...
    container.innerHTML = groups.map(([letter, start, end]) => {
        ...
        let html = `<div class="calling-number-row">
            <div class="calling-letter ${key}">${letter}</div>`;

        for (let n = start; n <= end; n++) {
            html += `<div class="calling-number ${called.has(n) ? "called" : ""}">${n}</div>`;
        }
...
function renderCalling(data, myCards) {
    ...
    document.getElementById("gameStatus").className =
        `room-status ${String(round.status || "PLAYING").toLowerCase()}`;

    document.getElementById("lastCalled").textContent =
        data.last_called ? bingoLabel(data.last_called) : "—";
    ...
    document.getElementById("gamePrize").textContent =
        money(data.financials?.prize_pool ?? round.prize_pool ?? 0);

    document.getElementById("gameCountdown").textContent =
        round.status === "COUNTDOWN"
            ? `${Number(data.countdown_remaining || 0)}s`
            : (round.status === "PLAYING" ? "LIVE" : "—");

    renderCallingBoard(called);
    renderWinnerPanel(data, myCards);

    // IMPORTANT: Only the current player's cartels are rendered here.
    // Other players' cartels are never exposed in the calling screen.
    ...
    if (myCards.length) {
        myCards.forEach(card => {
            cards.appendChild(
                renderMiniCard(card, called, card._winningPattern || null)
            );
        });
    } else {
        cards.innerHTML = `
            <div class="notice">
                ...
                Other players' cartels are hidden.
...
async function pollGame() {
    ...
    try {
        const data = await api(
            `/api/bingo/rounds/${currentGameRoundId}/state`
        );
        ...
        const status = String(
            data.status || data.round?.status || ""
        ).toUpperCase();

        if (["WINNER", "RESULT", "FINISHED"].includes(status) &&
            (data.winners || []).length) {

            // Give every player a short, stable winner popup period.
            if (!winnerPopupTimer) {
                winnerPopupTimer = setTimeout(async () => {
                    closeWinnerPopup();

                    const roomId = currentGameRoomId;
                    if (roomId) {
                        try {
                            await openCartelPage(roomId);
                        } catch (error) {
                            showToast(error.message);
                        }
                    }
