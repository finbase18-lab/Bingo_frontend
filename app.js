...
document.addEventListener("DOMContentLoaded",()=>{ensureAdminStyles();document.querySelectorAll('.admin-cat').forEach(b=>b.addEventListener('click',()=>selectAdminCategory(b.dataset.adminCategory)));document.getElementById('editGeneralSettings')?.addEventListener('click',()=>setSettingsEdit(true));document.getElementById('saveGeneralSettings')?.addEventListener('click',saveGeneralSettings);document.getElementById('editPatterns')?.addEventListener('click',()=>{document.querySelectorAll('.pattern-edit').forEach(x=>x.disabled=false);editPatterns.disabled=true;savePatterns.disabled=false});document.getElementById('savePatterns')?.addEventListener('click',async()=>{try{for(const el of document.querySelectorAll('.pattern-edit'))await adminAction(`/api/admin/settings/patterns/${el.dataset.key}`,{enabled:el.checked});document.querySelectorAll('.pattern-edit').forEach(x=>x.disabled=true);editPatterns.disabled=false;savePatterns.disabled=true;showToast('Winning patterns saved successfully')}catch(e){showToast(e.message)}});document.getElementById('searchPlayers')?.addEventListener('click',loadAdminPlayers);document.getElementById('refreshPlayers')?.addEventListener('click',loadAdminPlayers);document.getElementById('sendBroadcast')?.addEventListener('click',sendBroadcast);document.getElementById('copyInviteLink')?.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(inviteLink.value);showToast('Invite link copied')}catch(e){showToast('Copy failed')}});document.getElementById('shareInviteLink')?.addEventListener('click',()=>{if(inviteLink.value)window.open(`https://t.me/share/url?url=${encodeURIComponent(inviteLink.value)}&text=${encodeURIComponent('Join me in Bingo Arena!')}`,'_blank')})});
