...
function renderCartelSelection(data) {
    ...
    if (["PLAYING","WINNER","RESULT"].includes(round.status)) openCallingPage(Number(room.id));
}
...
async function openCartelPage(roomId) {
    ...
    } catch(error) { showToast(error.message); setPage("dashboardPage"); }
}
...
function renderCalling(data, myCards) {
    ...
    const players=new Set((data.players || []).map(p=>Number(p.user_id))).size;
    ...
    document.getElementById("gamePlayers").textContent=players;
    ...
    if (myCards.length) myCards.forEach(card=>cards.appendChild(renderMiniCard(card,called,card._winningPattern || null)));
...
async function pollGame() {
    ...
    try {
        ...
        if (data.status === "FINISHED") {
            ...
            setTimeout(()=>openCartelPage(roomId),500);
        }
    } catch(error) { showToast(error.message); }
}
